...
impl Aabb {
    ...
    #[inline] pub fn empty() -> Self { Self { from: Vec3A::MAX, to: Vec3A::MIN } }
    ...
    fn contains_point(&self, p: Vec3A) -> bool {
        self.from.x < p.x && p.x <= self.to.x &&
        self.from.y < p.y && p.y <= self.to.y &&
    ...
    pub fn intersects(&self, ray: &Ray) -> bool {
        let (xtmin, xtmax) = Self::check_axis(ray.origin.x, ray.direction.x, self.from.x, self.to.x);
        let (ytmin, ytmax) = Self::check_axis(ray.origin.y, ray.direction.y, self.from.y, self.to.y);
        let (ztmin, ztmax) = Self::check_axis(ray.origin.z, ray.direction.z, self.from.z, self.to.z);
